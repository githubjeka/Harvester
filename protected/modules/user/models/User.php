...
class User extends CActiveRecord
{
    ...
    public function relations()
    {
        ...
        if (isset(Yii::app()->getModule('user')->relations)) {
            $relations = array_merge(
                $relations,
                Yii::app()->getModule('user')->relations
            );
        }
        return $relations;
    }
    ...
    public function attributeLabels()
    {
        return array(
            ...
            'rowPerPage' => UserModule::t("rowPerPage"),
        );
    }
    ...
    {
        return array(
            'select' => 'id, username, email, createtime, lastvisit, superuser, status, rowPerPage',
        );
    }
    ...
    public static function getResultsPerPage()
    {
        if (Yii::app()->user->isGuest) {
            if (isset(Yii::app()->request->cookies['rowPerPage'])) {
                return Yii::app()->request->cookies['rowPerPage']->value;
            } else {
                return 10;
            }
        }
        $user = User::model()->find("id=" . Yii::app()->user->id);
        return $user->rowPerPage;
    }
}